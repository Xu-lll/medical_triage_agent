...
- 医学知识库：50 余条疾病、症状、药品、分诊规则，覆盖常见线上问诊科室。
- 向量数据库：使用 `langchain_chroma.Chroma` 持久化本地医学知识向量。
- 本地 LLM：使用 Ollama 中的 `deepseek-r1:7b` 主导多轮问诊回复、阶段性建议和动态追问。
...
- LLM 主导问诊：默认由大模型结合历史对话、检索知识和安全约束生成完整回复；规则逻辑只负责急症兜底、证据整理和 LLM 失败时的备用回答。
- 双模式对话：支持“纯 LLM 对话”和“RAG 知识库增强”。纯 LLM 模式不调用本地知识库，更像通用聊天问诊；RAG 模式会额外使用 SQLite/Chroma 医学知识库作为依据。
- 对话式问诊：Web 端支持连续追问、清空会话；后端按 `session_id` 隔离不同浏览器的上下文。
- 意图识别：能区分初始症状描述、病情补充、用药咨询和普通聊天，不再每一轮都输出固定格式分诊报告。
- 主动追问：在信息不足或需要进一步判断时，会询问病程、严重程度、伴随症状、年龄、过敏史和既往病史等关键问题。
...
页面中的“问诊模式”可以选择：
- 纯 LLM 对话：不检索本地知识库，完全由 LLM 根据上下文生成回复，适合测试自然对话能力。
- RAG 知识库增强：先检索本地医学知识库，再由 LLM 结合证据生成回复，适合需要可解释分诊依据的演示。
